...
class TestsUtilsTest(LogCaptureTestCase):
	...
	def testLazyLogging(self):
		...
		if unittest.F2B.log_lazy:
			...
			self.assertRaisesRegex(Exception, 'not all arguments converted', lambda: self.assertNotLogged('test'))
		else: # pragma: no cover
			# wrong logging syntax will throw an error directly:
			self.assertRaisesRegex(Exception, 'not all arguments converted', lambda: logSys.debug('test', 1, 2, 3))
