odoo.define('l10n_de_pos_cert.pos', function(require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        _triggerFiskalyFlushErrorPopup(error) {
            const message = {
                'noInternet': _t('Cannot sync the orders with Fiskaly !'),
                'unknown': _t('An unknown error has occurred ! Please contact Odoo for more information.')
            };
            this.chrome.showFiskalyErrorPopup(error, message);
        },
        //@Override
        ...
        async _flush_orders(orders, options) {
            ...
            if (result && fiskalyFailure.length === 0) {
                ...
                if (odooError) {
                    ...
                    this._triggerFiskalyFlushErrorPopup(fiskalyError);
                    fiskalyError.source = 'Fiskaly'; // This is used for _handleFailedPushForInvoice
