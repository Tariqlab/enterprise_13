    ...
    def _query_followup_level(self, all_partners=False):
        sql = """
            ...
            LEFT OUTER JOIN LATERAL (
                ...
                LEFT OUTER JOIN account_followup_followup_line ful ON ful.id = line.followup_line_id
                WHERE line.partner_id = partner.id
                  ...
                  AND COALESCE(line.date_maturity, line.date) + COALESCE(ful.delay, -999) <= CURRENT_DATE
                LIMIT 1
            ...
            LEFT OUTER JOIN LATERAL (
                ...
                WHERE line.partner_id = partner.id
                  ...
                  AND line.blocked IS FALSE
                  AND line.company_id = %(company_id)s
                  AND COALESCE(line.date_maturity, line.date) <= CURRENT_DATE
                LIMIT 1
