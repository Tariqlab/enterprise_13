...
class SocialLivePostFacebook(models.Model):
    ...
    def _post_facebook(self):
        for live_post in self:
            ...
            post_endpoint_url = url_join(self.env['social.media']._FACEBOOK_ENDPOINT, "/v10.0/%s/feed" % account.facebook_account_id)
            ...
            if post.image_ids and len(post.image_ids) == 1:
                ...
                photos_endpoint_url = url_join(self.env['social.media']._FACEBOOK_ENDPOINT, '/v10.0/%s/photos' % account.facebook_account_id)
            ...
            if (result.status_code == 200):
                ...
                live_post.facebook_post_id = result_json.get('post_id', result_json.get('id', False))
                values = {
                    'state': 'posted',
