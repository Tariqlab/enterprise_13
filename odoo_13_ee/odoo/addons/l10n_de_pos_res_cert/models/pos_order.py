...
class PosOrder(models.Model):
    ...
    def _order_fields(self, ui_order):
        ...
        if self._check_config_germany_floor(
                session_id=ui_order['pos_session_id']) and 'l10n_de_fiskaly_time_start' not in fields:
    ...
    def _line_differences(self, existing_order, ui_order=None):
        ...
        old_lines = []
        if existing_order:
            old_lines = existing_order.lines.read(['qty', 'product_id', 'price_subtotal_incl','price_unit', 'discount'], load=None)
    ...
    def _merge_order_lines(self, order_lines):
        ...
        for k, g in groupby(sorted(order_lines, key=keys), key=keys):
            ...
            line_dict[k] = {
                ...
                'text': self.env['product.product'].browse(k[0]).product_tmpl_id.name,
