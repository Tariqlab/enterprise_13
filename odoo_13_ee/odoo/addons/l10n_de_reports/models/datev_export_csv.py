    ...
    def _auto_init(self):
                ...
                UPDATE account_move
                  ...
                  FROM (
                          SELECT l.move_id mid,
                           ...
                           WHERE m.type in ('out_invoice', 'out_refund', 'in_refund', 'in_invoice', 'out_receipt', 'in_receipt')
        ...
        cr.execute(
            ...
            UPDATE account_move
              ...
              FROM (
                    SELECT m.id mid,
                           j.default_debit_account_id aid
                     FROM account_move m
                     JOIN account_journal j
                       ON m.journal_id = j.id
                    WHERE j.type = 'bank'
                      AND j.default_debit_account_id IS NOT NULL
                   ) r
             WHERE id = r.mid
              ...
              FROM (
                    SELECT l.move_id AS mid,
                     ...
                     WHERE j.type='general'
                       AND l.account_id in (j.default_debit_account_id, j.default_credit_account_id)
                     GROUP BY l.move_id,
                              l.account_id
                    HAVING count(*)=1
                   ) r
