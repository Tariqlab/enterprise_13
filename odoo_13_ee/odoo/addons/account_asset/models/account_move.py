...
class AccountMove(models.Model):
    ...
    def post(self):
        ...
        self._delete_reversed_entry_assets()
    ...
    def button_draft(self):
        for move in self:
            ...
            # Only affect the value when going from posted=>draft, and do not when going from canceled=>draft
            if move.asset_id and move.state == 'posted':
                move.asset_id.value_residual += abs(sum(move.line_ids.filtered(lambda l: l.account_id == move.asset_id.account_depreciation_id).mapped('balance')))
    ...
    def _delete_reversed_entry_assets(self):
        for move in self.filtered(lambda m: m.reversed_entry_id):
            reversed_products = move.line_ids.mapped(lambda l: (l.product_id, l.quantity))
            for line in move.reversed_entry_id.line_ids.filtered(lambda l: l.asset_id and (l.product_id, l.quantity) in reversed_products):
