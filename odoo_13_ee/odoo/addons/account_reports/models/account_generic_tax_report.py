...
class generic_tax_report(models.AbstractModel):
    ...
    def _get_options(self, previous_options=None):
        ...
        if not self.env['account.tax.report.line'].search_count([('country_id', '=', self.env.company.get_fiscal_country().id)]):
    ...
    def _generate_tax_closing_entry(self, options, move=False, raise_on_empty=False):
        ...
        if not len(line_ids_vals):
            tax_out_account_id = self.env['account.tax'].with_context(allowed_company_ids=company.ids)\
                .search([('type_tax_use', '=', 'sale')], limit=1)\
                .invoice_repartition_line_ids.account_id
            tax_in_account_id = self.env['account.tax'].with_context(allowed_company_ids=company.ids)\
    ...
    def _get_grids_refund_sql_condition(self):
        ...
        """
        return "account_move.tax_cash_basis_rec_id IS NULL AND account_move.type in ('in_refund', 'out_refund')"
    ...
    def _get_lines_by_grid(self, options, line_id, grids):
        ...
        country = self.env.company.get_fiscal_country()
    ...
    def _compute_tax_report_data(self, options):
        ...
        search_domain = options.get('tax_grids') and [('country_id', '=', self.env.company.get_fiscal_country().id)] \
                        or [('company_id', 'in', self.env.companies.ids if options.get('multi_company') else self.env.company.ids)]
