...
class TestAccountReports(TestAccountReportsCommon):
    ...
    def test_general_ledger_foreign_currency_account(self):
        ...
        invoice = self.env['account.move'].create({
            ...
            'currency_id': self.env.ref('base.EUR').id,
            'invoice_date': self.mar_year_minus_1,
            'date': self.mar_year_minus_1,
        ...
        self.assertLinesValues(
            ...
            [
                # Accounts.
                ('101401 Bank',                         '',             800.00,         1750.00,        -950.00),
                ('121000 Account Receivable',           '',             3375.00,        800.00,         2575.00),
                ('131000 Tax Paid',                     '',             705.00,         0.00,           705.00),
                ('211000 Account Payable',              '',             1750.00,        5405.00,        -3655.00),
                ('251000 Tax Received',                 '',             0.00,           375.00,         -375.00),
                ('400000 Product Sales',                '',             0.00,           1300.00,        -1300.00),
                ('600000 Expenses',                     '',             1100.00,        0.00,           1100.00),
                ('999999 Undistributed Profits/Losses', '',             3600.00,        1200.00,        2400.00),
                ('test foreign_currency_account',       -1000.0,        0.00,           500.00,         -500.00),
                # Report Total.
                ('Total',                               '',             11330.00,       11330.0,        0.00),
            ],
            currency_map={4: {'currency': self.env.ref('base.EUR')}},
        )
        ...
        self.assertLinesValues(
            ...
            [   0,                                      1,              3,              4,          5,              6,              7],
            [
                ('test foreign_currency_account',       '',             '',             -1000.00,   0.00,           500.00,         -500.00),
                ('Initial Balance',                     '',             '',             '',         0.00,           0.00,           0.00),
                ('INV/2017/0008',                       '03/01/2017',   'partner_a',    -1000.00,   '',             500.00,         -500.00),
                ('Total',                               '',             '',             -1000.00,   0.00,           500.00,         -500.00),
            ],
            currency_map={4: {'currency': self.env.ref('base.EUR')}},
        )
    ...
    def test_reconciliation_report_journal_company_currency(self):
        ''' Test the lines in a multi-company/multi-currency, with currency on the journal specified as the company's currency. '''
        bank_journal = self.env['account.journal'].search([('company_id', '=', self.company_child_eur.id), ('type', '=', 'bank')])
    ...
    def test_consolidated_journals_two_same_accounts(self):
        """
            When two journals use a common account, the values of it should be properly distinguished in the report.
        """
        # Duplicate the customer invoices journal, and add an invoice to it to get a balance for that
        # journal/account combination
        customer_invoice_journal = self.env['account.journal'].search([
            ('company_id', '=', self.company_parent.id),
            ('type', '=', 'sale')
        ], limit=1)
        journal_copy = customer_invoice_journal.copy()

        ci_journal_line_id = 'journal_%s' % customer_invoice_journal.id
        copy_journal_line_id = 'journal_%s' % journal_copy.id

        invoice = self.env['account.move'].create({
            'type': 'out_invoice',
            'journal_id': journal_copy.id,
            'partner_id': self.partner_a.id,
            'invoice_date': self.mar_year_minus_1,
            'date': self.mar_year_minus_1,
            'invoice_line_ids': [(0, 0, {
        ...
        # Unfold both journals using the same account, and make sure that the values are correct.
        options['unfolded_lines'] = [ci_journal_line_id, copy_journal_line_id]
        report = report.with_context(report._set_context(options))
        ...
        self.assertLinesValues(
            ...
            [
                ...
                ('Vendor Bills (BILL)',                 1265.00,        1265.00,        0.00),
                ('Bank (BNK1)',                         1350.00,        1350.00,        0.00),
                ('Customer Invoices (copy) (INV ()',    1000.00,        1000.00,        0.00),
                ('121000 Account Receivable',           1000.00,        0.00,           1000.00),
                ('400000 Product Sales',                0.00,           1000.00,        -1000.00),
                ('Total',                               5110.00,        5110.00,        0.00),
                ('',                                    '',             '',             ''),
                ('Details per month',                   '',             '',             ''),
                ('Jan 2017',                            1160.00,        1160.00,        0.00),
                ('Feb 2017',                            1170.00,        1170.00,        0.00),
                ('Mar 2017',                            2780.00,        2780.00,        0.00),
            ],
        )
    ...
    def test_automatic_vat_closing_entry_no_operation_on_period(self):
        ...
        with patch.object(type(report), '_get_vat_report_attachments', autospec=True, side_effect=_get_attachment):
            # Try closing VAT in two different period without any invoice in each period.
            ...
            move = self._close_vat_entries(report, options)
            # assert element on move
            self.assertEqual(len(move.line_ids), 2, 'Tax Move created should have 2 lines')
            self.assertRecordValues(move.line_ids, [
                {'debit': 0, 'credit': 0, 'balance': 0},
                {'debit': 0, 'credit': 0, 'balance': 0}
            ])
